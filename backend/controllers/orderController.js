...
const verifyPaystack = async (req, res) => {
  try {
    const { reference, orderId, userId } = req.body; // userId comes from authUser middleware
    ...
    res.json({ 
      success: false, 
      message: error.response?.data?.message || error.message || "Verification error occurred"
    });
  }
};
